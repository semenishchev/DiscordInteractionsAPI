...
public class TicketUtil {
    ...
    public static FileUpload getTranscript(TextChannel channel) throws IOException {
        File folder = new File("transcripts");
        if(!folder.exists()) {
            folder.mkdirs();
        }

        File transcriptFile = new File(folder, channel.getId() + "-" + channel.getName() + ".html");
        String transcript = createTranscript(channel);
        if(transcript == null) {
            return null;
        }
        Files.writeString(transcriptFile.toPath(), transcript);
        return FileUpload.fromData(transcriptFile);
    }

    public static String createTranscript(TextChannel channel) throws IOException {
        List<Message> messages = MessageHistory.getHistoryFromBeginning(channel).complete().getRetrievedHistory();
        if (messages.isEmpty()) {
            return null;
        }
        Document document = Jsoup.parse(new String(ConstantPool.CLASS_LOADER.getResourceAsStream("transcript-template.html").readAllBytes()), "UTF-8");
